...
<style>
.audit-fields thead{display:none}.audit-fields tr{display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:10px;padding:14px;border-bottom:1px solid var(--line)}.audit-fields td{display:block;padding:4px;border:0}.audit-fields input{width:100%!important}.audit-fields small.field-label{display:block;color:var(--muted);margin-bottom:5px}.audit-recap{position:sticky;bottom:8px;padding:12px;background:var(--panel);border:1px solid var(--brand);border-radius:14px;z-index:2}.audit-stepper{display:grid;grid-template-columns:48px 1fr 48px;gap:6px}.audit-stepper button{min-height:48px;padding:0;font-size:24px}.audit-search{position:sticky;top:6px;z-index:3;background:var(--panel);padding:8px 0}.audit-hidden{display:none!important}@media(max-width:640px){.audit-fields tr{grid-template-columns:1fr}.audit-fields input{min-height:48px}.audit-recap{font-size:14px}}
</style>
<form method="post" enctype="multipart/form-data" action="/audit-externe/rapports/<?= (int) $report['id'] ?>/brouillon" data-audit-form>
    ...
    <div class="grid" style="grid-template-columns:repeat(auto-fit,minmax(220px,1fr))">
        <label><?= $entryMode === 'serveur' ? 'Montant total que je declare avoir vendu aujourd hui' : 'Vente declaree' ?> <input data-declared type="number" min="0" step="0.01" name="declared_sales" value="<?= e($report['declared_sales']) ?>"></label>
    ...
    <label class="audit-search">Rechercher un produit <input type="search" data-product-search placeholder="Nom ou categorie" autocomplete="off"></label>
    ...
    <?php foreach ($products as $product): $saved = null; foreach ($items as $candidate) { if ((int) $candidate['product_id'] === (int) $product['id']) { $saved = $candidate; break; } } ?>
        <tr data-price="<?= e((string) $product['sale_price']) ?>" data-search="<?= e(strtolower($product['name'] . ' ' . $product['category_name'])) ?>"><td><strong><?= e($product['name']) ?></strong><br><small><?= e($product['category_name']) ?><?= $entryMode === 'serveur' ? '' : ' · prix fige ' . number_format((float) $product['sale_price'], 0, ',', ' ') ?></small></td>
        <?php foreach ($fields as $field): ?>
            <td><small class="field-label"><?= e($fieldLabels[$field]) ?></small><div class="<?= $entryMode === 'serveur' && $field === 'sold_quantity_declared' ? 'audit-stepper' : '' ?>"><?php if ($entryMode === 'serveur' && $field === 'sold_quantity_declared'): ?><button type="button" data-step="-1" aria-label="Retirer une unite">−</button><?php endif; ?><input data-field="<?= e($field) ?>" title="<?= e($fieldLabels[$field]) ?>" style="width:105px" type="number" min="0" step="0.001" name="items[<?= (int) $product['id'] ?>][<?= e($field) ?>]" value="<?= e((string) ($saved[$field === 'purchase_unit_price' ? 'purchase_price_snapshot' : $field] ?? ($field === 'previous_stock' && $product['previous_stock_default'] !== null ? $product['previous_stock_default'] : '0'))) ?>"><?php if ($entryMode === 'serveur' && $field === 'sold_quantity_declared'): ?><button type="button" data-step="1" aria-label="Ajouter une unite">+</button><?php endif; ?></div></td>
        <?php endforeach; ?>
    ...
    <div class="audit-recap">Resume dynamique : <strong data-recap>0 unite · calcule 0 FC · declare 0 FC · ecart 0 FC</strong></div>
    <button type="submit">Enregistrer le brouillon</button>
</form>
<script>
(()=>{const f=document.querySelector('[data-audit-form]');if(!f)return;const update=()=>{let q=0,a=0;f.querySelectorAll('tr[data-price]').forEach(r=>{const s=Number(r.querySelector('[data-field="sold_quantity_declared"]')?.value||0);q+=s;a+=s*Number(r.dataset.price||0);const t=r.querySelector('[data-field="purchase_total"]'),u=r.querySelector('[data-field="purchase_unit_price"]'),n=Number(r.querySelector('[data-field="purchased_quantity"]')?.value||0);if(t&&u&&document.activeElement===t&&n>0)u.value=(Number(t.value||0)/n).toFixed(2)});const d=Number(f.querySelector('[data-declared]')?.value||0),gap=d-a;f.querySelector('[data-recap]').textContent=q.toLocaleString('fr-FR')+' unite · calcule '+a.toLocaleString('fr-FR')+' FC · declare '+d.toLocaleString('fr-FR')+' FC · ecart '+gap.toLocaleString('fr-FR')+' FC'};f.addEventListener('click',e=>{const b=e.target.closest('[data-step]');if(!b)return;const i=b.parentElement.querySelector('input');i.value=Math.max(0,Number(i.value||0)+Number(b.dataset.step));i.dispatchEvent(new Event('input',{bubbles:true}))});f.addEventListener('input',update);const search=f.querySelector('[data-product-search]');search?.addEventListener('input',()=>{const term=search.value.trim().toLocaleLowerCase('fr');f.querySelectorAll('tr[data-search]').forEach(r=>r.classList.toggle('audit-hidden',term!==''&&!r.dataset.search.includes(term)))});update()})();
</script>
</section>
<section class="card" style="padding:22px;margin-bottom:22px">
